...
public class bl_AudioPlayer : MonoBehaviour {
    ...
    public void SetAudioList(string targetList)
    {
        switch(targetList)
        {
            case "KBA_CD1":
    ...
    public void SetCurrentClip(int number)
    {
        //CurrentClip = number;
        NewClip(m_Clip[number - 1]);
    ...
    public void PlayPause()
    {
        ...
        if (m_Source.clip == null)
        {
            if (m_Clip != null) { NewClip(m_Clip[CurrentClip]); } else { Debug.Log("Dont have clip yet!"); return; } 
        }
